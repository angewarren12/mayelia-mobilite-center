        ...
        <form method="POST" action="{{ route('agents.update', $agent) }}">
            ...
            <div class="space-y-6">
                ...
                <div>
                   ...
                   <select id="role" 
                           name="role" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-mayelia-500 focus:border-transparent @error('role') border-red-500 @enderror">
                       <option value="agent" {{ old('role', $agent->role) == 'agent' ? 'selected' : '' }}>Agent d'accueil (Dossiers)</option>
                       <option value="agent_biometrie" {{ old('role', $agent->role) == 'agent_biometrie' ? 'selected' : '' }}>Agent Biométrie</option>
                   </select>
                   <p class="mt-1 text-xs text-gray-500">
                       L'agent biométrie a un accès restreint uniquement à la file d'attente biométrique.
                   </p>
                </div>
                ...
                <div id="biometrie-info" class="hidden bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                Pour un <strong>Agent Biométrie</strong>, aucune permission manuelle n'est nécessaire.
                                <br>Cet agent aura automatiquement accès à son interface dédiée de gestion de file d'attente biométrique.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const biometrieInfo = document.getElementById('biometrie-info');
                    
                    function togglePermissions() {
                        if (roleSelect.value === 'agent_biometrie') {
                            permissionsSection.classList.add('hidden');
                            biometrieInfo.classList.remove('hidden');
                            // Note: En édition, on ne décoche pas forcément tout, car si on revient à 'agent' on veut retrouver les permissions
                            // Mais si on soumet en agent_biometrie, les permissions cochées ne seront pas utilisées par le système
                            // donc c'est acceptable.
                        } else {
                            permissionsSection.classList.remove('hidden');
                            biometrieInfo.classList.add('hidden');
                        }
                    }
